...
from .models import RelyInvoice, Status, RelyProcessed, RelyCompleted, RelyPaid, RelyProblem, RelyReassigned
...
def rely_invoice_processed(request):
    ...
    # Calculate overall average days difference
...
def rely_invoice_reassign(request):
    ...
    if request.method == "POST":
        ...
        messages.success(request, f"You searched {inv_data}")
        if inv_data:
            search_results = RelyReassigned.objects.filter(dispatch_number__icontains=inv_data) | \
